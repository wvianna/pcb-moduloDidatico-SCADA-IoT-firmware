...
// Note 163: O pino de AP trigger compartilhado com PWM da resistencia de aquecimento exige cuidado no boot.
// Note 164: Compartilhamento e aceitavel quando leitura de trigger ocorre apenas na partida.
// Note 165: Em hardware final, pino dedicado de setup aumenta robustez operacional.
...
constexpr uint8_t PIN_AP_TRIGGER = D1;     // GPIO5 (amostrado so no boot, ativo em nivel baixo)
constexpr uint8_t PIN_INPUT_0 = D8;        // GPIO15 (entrada discreta 10001)
constexpr uint8_t PIN_INPUT_1 = 3;         // RX / GPIO3
...
constexpr uint8_t PIN_PWM_HEATER = D1;       // GPIO5
...
String htmlPage(const String &status, const String &message = "") {
  ...
  html += "<tr><td>PWM Resistencia de aquecimento</td><td>D1</td><td>GPIO5</td><td>Holding register 40003 (0-1023, compartilhado AP trigger boot)</td></tr>";
